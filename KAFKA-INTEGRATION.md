...
Kafka 테이블 엔진을 실무에 적용하기 전에 반드시 알아야 할 네 가지 경계 상황을
실제로 재현해봤습니다. 전부 `kafka_demo` 안에서 별도 토픽/테이블로 격리해
진행했고, 실험 후 바로 정리했습니다.

### 1) 하나의 Kafka 엔진 테이블을 여러 MV가 읽으면? (fan-out)

같은 `edge_fanout_queue`(Kafka 엔진)를 읽는 MV 두 개(`edge_fanout_mv_a` →
`edge_fanout_a`, `edge_fanout_mv_b` → `edge_fanout_b`)를 만들고 메시지 5건을
발행했습니다.

**결과: 두 타깃 테이블 모두 5건씩 전부 받았습니다** — 메시지를 나눠 갖는 게
아니라 각 MV가 독립적으로 전체를 받습니다. `system.kafka_consumers`의
`num_messages_read`도 `5`(10이 아님)였습니다. 즉 ClickHouse는 Kafka에서
**한 번만 읽어 내부 블록으로 만들고, 그 블록을 부착된 모든 MV에 각각
전달**합니다(일반 테이블에 여러 MV를 붙였을 때와 동일한 fan-out 방식).
같은 스트림을 서로 다른 두 가지 집계/가공 방식으로 쓰고 싶다면(예: 원본 저장용
MV 하나 + 실시간 집계용 MV 하나) 컨슈머를 늘리지 않고 MV만 여러 개 붙이면
됩니다.

### 2) MV가 없거나 삭제되면 컨슈밍은 어떻게 되는가

**MV가 처음부터 없는 경우**: Kafka 엔진 테이블만 만들고 메시지 3건을 발행했더니,
`system.kafka_consumers.is_currently_used = 0`, `num_messages_read = 0`이었고
컨슈머 그룹 자체가 `Dead`(가입도 안 함) 상태였습니다. **Kafka 엔진 테이블은
MV가 최소 1개 이상 붙어 있어야만 백그라운드 컨슈밍 스레드가 실제로 동작**합니다
— 테이블만 만들어두는 건 아무 일도 하지 않습니다.

**중간에 MV를 DROP하는 경우**: MV를 붙여 정상 소비 중인 상태에서 `DROP TABLE
... mv`로 MV만 제거하고, 백그라운드 루프가 "뷰가 0개"임을 인지할 시간(약
15초, `stream_flush_interval_ms`인 7.5초보다 넉넉히)을 준 뒤 새 메시지를
발행했습니다.

```
GROUP        edge_no_mv2_group
STATE        Stable
TOTAL-LAG    2      -- 커밋된 오프셋이 새 메시지 2건만큼 뒤처져 멈춰있음(유실 아님)
```

**메시지가 유실되지도, 오프셋이 헛되이 전진하지도 않았습니다** — 컨슈머
그룹의 커밋 오프셋이 그 자리에서 멈춰 새 메시지 2건이 `LAG`로 고스란히 남아
있었습니다. 이후 MV를 다시 `CREATE`하니 밀렸던 메시지가 **중복도 유실도 없이**
정확히 도착했습니다. 즉 **MV를 없애는 건 위험한 "블랙홀"이 아니라 안전한
일시정지**입니다 — 다만 DROP 직후 짧은 시간(한 폴링 주기 내) 동안은 이미
진행 중이던 폴링 사이클이 완료될 때까지 컨슈밍이 즉시 멈추지 않을 수 있다는
점(첫 시도에서 DROP 후 4초 만에 확인했을 때는 그사이 발행한 메시지까지 함께
들어왔습니다)도 함께 확인했습니다 — DDL이 반영되는 시점과 백그라운드 폴링
루프의 상태 사이에 미세한 지연이 있다는 뜻입니다.

### 3) 스키마가 안 맞는 메시지: 필드 누락/여분 필드/타입 오류

`event_id UInt64, event_type String, payload String` 스키마에 대해 다섯 가지
메시지를 순서대로 발행했습니다: ①정상 ②`payload` 필드 누락 ③여분 필드
(`unexpected`) 추가 ④`event_id`에 숫자 대신 문자열 ⑤(④ 이후에도 진행되는지
확인용) 정상 메시지.

`kafka_skip_broken_messages = 0`(기본값)으로는 ④에서 파싱 에러가 나서
컨슈밍이 그 자리에 멈췄습니다(앞서 겪은 poison pill과 동일 증상). **그런데
이번엔 타깃 테이블이 완전히 비어 있었습니다** — ①②③처럼 문제없이 파싱될
메시지들조차 하나도 들어오지 않았습니다. 원인은 `stream_flush_interval_ms`
(기본 7500ms): **Kafka 엔진은 이 주기 동안 폴링한 메시지들을 하나의 블록으로
묶어 한 번에 INSERT합니다.** 5건을 8초 안에 몰아서 발행했기 때문에 전부 같은
블록에 들어갔고, 그 블록에 파싱 불가능한 메시지가 하나만 있어도 **블록 전체가
커밋되지 않고 무한 재시도**됩니다 — 같은 블록 안의 "멀쩡한" 메시지들까지
전부 지연/보류된다는 뜻입니다.

`kafka_skip_broken_messages = 10`으로 테이블을 재생성(같은 컨슈머 그룹이라
커밋되지 않은 오프셋 0부터 재개)하자 ①②③⑤가 한꺼번에 도착했고 ④만 정확히
건너뛰었습니다:

```
event_id=1  event_type=normal          payload=ok     -- 정상
event_id=2  event_type=missing_field   payload=       -- 누락 필드는 타입 기본값(빈 문자열)으로 채워짐
event_id=3  event_type=extra_field     payload=ok     -- 여분 필드는 조용히 무시됨 (에러 아님)
event_id=5  event_type=after_bad_type  payload=ok     -- 4번(타입 오류)만 건너뜀
```

**필드 누락과 여분 필드는 애초에 에러가 아니라 정상 동작**(각각 기본값 채움 /
무시)이고, **진짜 위험한 건 타입 자체가 안 맞는 값**입니다. 그리고 이번 실험의
핵심 교훈은: `kafka_skip_broken_messages`가 걸러주는 건 "그 메시지 하나"가
아니라 **그 메시지가 포함된 블록 전체의 진행 차단**이라는 것 — 프로덕션에서
메시지 검증이 느슨한 프로듀서가 있다면, 정상 메시지들까지 최대
`stream_flush_interval_ms`만큼 지연될 수 있다는 걸 감안해야 합니다.

### 4) 컨슈밍 도중 ClickHouse 파드가 죽으면 데이터가 유실되거나 중복될까

2만 건짜리 메시지를 발행하면서, 발행 시작 2.5초 후(첫 `stream_flush_interval_ms`
7.5초가 지나기 전, 즉 아직 아무것도 커밋되지 않았을 시점을 노려) 컨슈밍 중이던
파드(`chi-chi-cluster1-0-0-0`)를 `--grace-period=0 --force`로 강제 삭제했습니다.

```
[t=0s]    파드 강제 삭제
[t=~4s]   CrashLoopBackOff (StatefulSet가 재생성한 새 파드가 기동 중)
[t=17s]   파드 Ready
[t=~25-30s] 컨슈밍 재개, 2만 건 전부 도착
```

```sql
SELECT count() AS total_rows, count(DISTINCT event_id) AS distinct_ids
FROM kafka_demo.edge_crash_events
-- 20000  20000   (중복 0건, 누락 0건)
```

**결과: 유실도 중복도 없었습니다.** 이번 타이밍(폴링 후 블록이 아직
커밋되지 않은 시점에 크래시)에서는 ClickHouse가 재시작 후 커밋되지 않은
오프셋부터 그대로 다시 읽어와 정확히 한 번씩만 처리했습니다. 다만 이건
**이 타이밍에서만 보장되는 결과**입니다 — Kafka 엔진의 오프셋 커밋은 "블록을
타깃 테이블에 INSERT한 이후"에 일어나므로, 이론적으로는 **INSERT는 이미
끝났는데 오프셋 커밋 직전에 크래시가 나는 훨씬 더 좁은(수 밀리초 단위) 구간**이
존재하며 그 경우엔 재시작 후 같은 메시지가 다시 읽혀 **중복 삽입**될 수
있습니다. 이 구간은 수동으로 타이밍을 맞춰 재현하기엔 너무 좁아 이번 실험에서
직접 재현하지는 못했지만, 이는 Kafka 엔진이 "정확히 한 번(exactly-once)"이
아니라 **"적어도 한 번(at-least-once)"**만 보장한다는 뜻입니다. 앞서
컨슈머 그룹 변경 실험에서 확인했듯 `MergeTree`는 중복 제거를 하지 않으므로,
크래시/재시작이 잦을 수 있는 프로덕션 환경이라면 타깃 테이블을
`ReplacingMergeTree` + 자연 키로 설계해 이런 드문 중복 가능성에도 안전하게
대응해야 합니다.

또한 이번 실험에서 **파드 강제 삭제 → 새 파드 Ready까지 약 17초**가
걸렸다는 점도 참고할 만합니다 — Keeper 연결·파트 로드 등 기동 절차 때문에,
GUIDE.md에서 확인한 "레플리카 하나가 죽어도 나머지가 트래픽을 받아
무중단"이라는 전제는 Kafka 컨슈머에는 그대로 적용되지 않습니다: 해당
파드가 유일하게 그 Kafka 엔진 테이블을 갖고 있었으므로, 파드가 죽어있던
약 17~30초 동안은 그 토픽에 대한 소비가 전면 중단됐습니다. Kafka 엔진
테이블 자체를 고가용으로 만들려면(레플리카마다 별도 컨슈머 그룹으로
중복 소비하거나, `ReplicatedMergeTree` 타깃 + 여러 레플리카에 동일한
Kafka 엔진 테이블을 두는 방식 등) 별도 설계가 필요합니다.
